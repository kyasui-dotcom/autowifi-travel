...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "海外旅行から帰国して、携帯電話の請求書を見て驚いた経験はありませんか？国際ローミングによる高額請求は、多くの旅行者が直面する問題です。この記事では、海外での通信費を大幅に節約する方法を具体的に解説します。eSIMをはじめとする代替手段を活用して、安心して旅行を楽しみましょう。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/save-money-roaming", title: c.title, description: c.intro.slice(0, 160) });
}
