...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "マレーシアはクアラルンプール（KL）のペトロナスツインタワー、ペナンのストリートアート巡りと世界遺産の街並み、ランカウイのビーチリゾートなど、多彩な魅力を持つ国です。通信インフラが整備されており、eSIMを使えばKLIA到着後すぐにGrab配車やGoogle Mapsでの移動が可能。マレーシアの通信料金はリーズナブルで、コスパの良い旅行が実現できます。",
    sections: [
      {
        title: "マレーシアの通信環境",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/malaysia-esim", title: c.title, description: c.intro.slice(0, 160) });
}
