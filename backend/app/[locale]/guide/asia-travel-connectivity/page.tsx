...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "アジアは世界でも最もモバイル通信インフラが発達している地域のひとつです。日本、韓国、シンガポールなどは5Gネットワークが充実し、タイやベトナムなど東南アジア諸国もLTE環境が急速に整備されています。本ガイドでは、アジア主要国の通信環境とeSIMの活用方法を詳しく解説します。本記事ではアジア各国のモバイル通信事情と、快適に旅するためのeSIM活用術・アジア主要国の通信環境・アジア旅行におすすめのeSIMプランなどを2026年時点の情報に基づき、旅行者目線で具体的に解説しています。",
    sections: [
      {
        title: "アジア主要国の通信環境",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/asia-travel-connectivity", title: c.title, description: truncateAtSentence(c.intro) });
}
