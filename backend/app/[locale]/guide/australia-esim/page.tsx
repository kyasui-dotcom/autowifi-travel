...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "オーストラリアは日本の約20倍の国土を持つ広大な国で、シドニーのオペラハウス、メルボルンのカフェ文化、グレートバリアリーフの自然など魅力が満載です。主要都市では高速な5G/4G通信が利用可能ですが、都市間の移動時にはカバレッジの変化に注意が必要です。eSIMを利用すれば、到着後すぐに通信を開始でき、オーストラリア旅行を最大限に楽しめます。本記事では広大なオーストラリアをeSIMで安心して旅しよう・オーストラリアのモバイル通信事情・おすすめのeSIMプランなどを2026年時点の情報に基づき、旅行者目線で具体的に解説しています。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/australia-esim", title: c.title, description: truncateAtSentence(c.intro) });
}
