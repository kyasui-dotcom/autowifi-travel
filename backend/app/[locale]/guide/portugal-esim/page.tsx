...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "ポルトガルはヨーロッパで人気急上昇中の観光地であり、リスボンやポルトを中心に高品質なモバイル通信が利用可能です。美しいアズレージョの街並みを散策しながら、eSIMがあればトラムの路線検索やレストラン予約もスムーズに行えます。EU圏内ローミング対応プランなら、隣国スペインへの周遊旅行にも便利です。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/portugal-esim", title: c.title, description: c.intro.slice(0, 160) });
}
