...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "カンボジアは世界遺産アンコールワットをはじめ、魅力的な観光スポットが豊富な東南アジアの人気旅行先です。近年モバイル通信インフラが急速に整備され、主要都市では4G LTEが安定して利用可能です。eSIMを利用すれば、プノンペン国際空港到着後すぐにGrabやPassAppで移動手段を確保し、アンコール遺跡群の情報検索も快適に行えます。本記事ではeSIMでカンボジア旅行をもっと便利に - アンコールワットからビーチリゾートまで・カンボジアのモバイル通信事情・おすすめのeSIMプランなどを2026年時点の情報に基づき、旅行者目線で具体的に解説しています。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/cambodia-esim", title: c.title, description: truncateAtSentence(c.intro) });
}
