...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "スリランカはコロンボのモダンな都市景観から、キャンディの仏教寺院、ハイキングで有名なエラ、そして美しいビーチまで多彩な魅力を持つ島国です。近年通信インフラが急速に発展しており、eSIMを利用すれば到着後すぐに快適なネット環境を手に入れられます。",
    sections: [
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/sri-lanka-esim", title: c.title, description: c.intro.slice(0, 160) });
}
