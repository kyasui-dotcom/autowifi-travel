...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "家族での海外旅行では、大人のスマートフォンだけでなく、子供のタブレットやゲーム機など複数のデバイスでインターネット接続が必要になります。本ガイドでは、家族旅行に最適なeSIMプランの選び方とデバイス管理のコツを解説します。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/family-travel-esim", title: c.title, description: c.intro.slice(0, 160) });
}
