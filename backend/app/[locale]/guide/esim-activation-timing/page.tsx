...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "eSIMを購入したあと、いつインストールして有効化すべきか迷う方は多いです。出発前にセットアップしておくべきか、現地到着後に有効化すべきか、プランの種類や有効期限を考慮した最適なタイミングを解説します。",
    sections: [
      {
        title: "インストールと有効化の違い",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/esim-activation-timing", title: c.title, description: c.intro.slice(0, 160) });
}
