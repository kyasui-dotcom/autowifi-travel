...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "eSIMの設定や利用中にトラブルが発生することがあります。この記事では、最もよくあるeSIMの問題とその解決方法をまとめました。QRコードが読み取れない、データ通信ができない、通信速度が遅いなど、状況別に対処法を解説します。ほとんどの問題は数分で解決できます。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/esim-troubleshooting", title: c.title, description: c.intro.slice(0, 160) });
}
