...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "メキシコは遺跡からビーチリゾートまで多彩な魅力を持つ人気の旅行先です。Telcel、AT&T Mexico、Movistarの3大キャリアが全国をカバーしており、eSIMを利用すればメキシコシティやカンクンの空港に到着後すぐにUberの手配やGoogle Mapsでの移動が可能です。事前にeSIMを設定しておけば、到着直後から安心してスマホを活用できます。本記事ではeSIMでメキシコ旅行を快適に。主要都市からビーチリゾートまで・メキシコのモバイル通信事情・おすすめのeSIMプランなどを2026年時点の情報に基づき、旅行者目線で具体的に解説しています。",
    sections: [
      {
        title: "メキシコのモバイル通信事情",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/mexico-esim", title: c.title, description: truncateAtSentence(c.intro) });
}
