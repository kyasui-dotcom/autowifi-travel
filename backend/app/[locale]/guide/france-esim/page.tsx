...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "フランスは世界有数の観光大国であり、パリを中心に高品質なモバイル通信が利用可能です。5Gも主要都市で展開が進んでおり、eSIMを利用すればシャルル・ド・ゴール空港到着後すぐにメトロの乗り換え検索やGoogleマップでの観光案内が可能です。EU圏内ローミング対応プランなら、近隣国への周遊旅行にも便利です。",
    sections: [
      {
        title: "フランスのモバイル通信事情",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/france-esim", title: c.title, description: c.intro.slice(0, 160) });
}
