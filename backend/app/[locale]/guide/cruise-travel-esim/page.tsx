...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "クルーズ旅行では通信環境が特殊です。船上WiFiは高額で低速なことが多く、寄港地では限られた時間で効率的にデータ通信を行う必要があります。本ガイドではクルーズ旅行者向けに、eSIMの最適な活用法と通信費の節約術を解説します。",
    sections: [
      {
        title: "クルーズ船の通信事情",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/cruise-travel-esim", title: c.title, description: c.intro.slice(0, 160) });
}
