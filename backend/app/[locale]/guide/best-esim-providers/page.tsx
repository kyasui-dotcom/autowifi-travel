...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "海外eSIMをおすすめ基準で比較したい人向けに、Airalo、Holafly、AutoWiFiなど主要サービスを料金、対応国数、データ品質、使いやすさ、サポート体制の観点から整理しました。短期旅行、周遊旅行、出張など用途別の選び方も分かります。本記事では主要eSIMサービスを比較して、旅行に合うおすすめプランを見つけるためのガイド・eSIMプロバイダー選びのポイント・主要プロバイダーの特徴などを2026年時点の情報に基づき、旅行者目線で具体的に解説しています。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/best-esim-providers", title: c.title, description: truncateAtSentence(c.intro) });
}
