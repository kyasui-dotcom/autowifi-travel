...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "トルコはヨーロッパとアジアにまたがる独特の国で、イスタンブールの歴史的なモスクやバザール、カッパドキアの幻想的な奇岩群と気球ツアーなど、唯一無二の旅行体験が待っています。トルコの通信インフラは主要都市で整備されており、eSIMを使えばイスタンブール空港到着後すぐにGoogle Mapsやタクシー配車アプリを利用できます。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/turkey-esim", title: c.title, description: c.intro.slice(0, 160) });
}
