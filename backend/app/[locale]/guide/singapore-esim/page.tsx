...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "シンガポールは世界トップクラスの通信インフラを持つ都市国家です。国土が小さいため、ほぼ全域で高速な4G/5G通信が利用可能。eSIMを使えば、チャンギ空港に到着した瞬間からGrab配車やGoogle Mapsでの移動が快適になります。マリーナベイ・サンズ、ガーデンズ・バイ・ザ・ベイ、セントーサ島など人気スポットでもストレスなくSNS投稿が楽しめます。本記事ではコンパクトな都市国家を最高速のモバイル通信で楽しもう・シンガポールの通信環境・eSIMのメリット：チャンギ空港到着後すぐ使えるなどを2026年時点の情報に基づき、旅行者目線で具体的に解説しています。",
    sections: [
      {
        title: "シンガポールの通信環境",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/singapore-esim", title: c.title, description: truncateAtSentence(c.intro) });
}
