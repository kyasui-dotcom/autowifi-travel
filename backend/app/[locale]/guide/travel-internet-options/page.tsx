...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "海外旅行中のインターネット接続にはいくつかの選択肢があります。eSIM、モバイルWiFi（ポケットWiFi）、国際ローミング、現地SIMカード、そして無料WiFiスポット。それぞれの特徴、料金、利便性を比較して、あなたの旅行スタイルに最適な方法を見つけましょう。本記事ではeSIM・WiFi・ローミング・SIMカード — あなたに合う方法は？・eSIM（トラベルeSIM）・モバイルWiFi（ポケットWiFi）などを2026年時点の情報に基づき、旅行者目線で具体的に解説しています。",
    sections: [
      {
        title: "eSIM（トラベルeSIM）",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/travel-internet-options", title: c.title, description: truncateAtSentence(c.intro) });
}
