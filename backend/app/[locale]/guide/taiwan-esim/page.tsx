...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "台湾は日本から近く、手軽に行ける海外旅行先として大人気です。台北の故宮博物院や夜市、九份の幻想的な街並み、高雄の港町の魅力を存分に楽しむには、安定したモバイル通信が欠かせません。台湾の通信インフラは非常に整備されており、eSIMを使えば桃園空港や松山空港に到着した瞬間から快適にインターネットを利用できます。本記事では夜市グルメから絶景スポットまで、eSIMで台湾旅行を満喫・台湾の通信環境と主要キャリア・台湾旅行でeSIMが便利な理由などを2026年時点の情報に基づき、旅行者目線で具体的に解説しています。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/taiwan-esim", title: c.title, description: truncateAtSentence(c.intro) });
}
