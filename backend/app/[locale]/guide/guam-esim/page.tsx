...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "グアムは日本から約3時間半で行けるアメリカ準州（US territory）です。USキャリアのネットワークが整備されているため、eSIMを使えばタモン湾のリゾートエリアからグアム南部の歴史的スポットまで、快適なモバイル通信が楽しめます。短い飛行時間と時差わずか1時間という利便性から、週末旅行にも人気のグアムでの通信事情を詳しく解説します。",
    sections: [
      {
        title: "グアムの通信環境",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/guam-esim", title: c.title, description: c.intro.slice(0, 160) });
}
