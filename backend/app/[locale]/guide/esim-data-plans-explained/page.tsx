...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "eSIMのデータプランには、データ容量、有効期間、対応エリアなど、さまざまな要素があります。適切なプランを選ぶことで、旅行中のインターネット環境を快適にしながら、無駄な出費を抑えることができます。この記事では、eSIMのデータプランの仕組みと選び方を詳しく解説します。",
    sections: [
      {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/esim-data-plans-explained", title: c.title, description: c.intro.slice(0, 160) });
}
