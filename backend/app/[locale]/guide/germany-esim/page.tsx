...
const CONTENT: Record<Locale, ArticleContent> = {
  ja: {
    ...
    intro: "ドイツはヨーロッパ最大の経済大国であり、ベルリンの歴史、ミュンヘンのビール文化、フランクフルトのビジネス街など多様な魅力を持つ国です。主要都市では4G LTEが安定しており、eSIMを利用すればドイツ鉄道の乗り換え検索や観光スポットの予約がスムーズに行えます。",
    sections: [
      {
        title: "ドイツのモバイル通信事情",
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale: locale as Locale, path: "/guide/germany-esim", title: c.title, description: c.intro.slice(0, 160) });
}
